...
func (f *PacketTypeField) MarshalBinary() (data []byte, err error) {
	...
	binary.BigEndian.PutUint16(data[2:], f.ns_type)
	return
}
func (f *PacketTypeField) UnmarshalBinary(data []byte) error {
	f.namespace = binary.BigEndian.Uint16(data[0:])
	f.ns_type = binary.BigEndian.Uint16(data[2:])
	return nil
}

func NewPacketTypeField(namespace uint16, ns_type uint16) *MatchField {
	f := new(MatchField)
	f.Class = OXM_CLASS_OPENFLOW_BASIC
	...
	packetTypeField.ns_type = ns_type
